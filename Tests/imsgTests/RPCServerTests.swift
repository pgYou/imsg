...
@Test
func rpcSendRejectsReplyToGuid() async throws {
  let store = try RPCTestDatabase.makeStore()
  let output = TestRPCOutput()
  let server = RPCServer(store: store, verbose: false, output: output)
...
@Test
func rpcSendRejectsReactionInAppleScript() async throws {
  let store = try RPCTestDatabase.makeStore()
  let output = TestRPCOutput()
  let server = RPCServer(store: store, verbose: false, output: output)

  let line =
    #"{"jsonrpc":"2.0","id":"5","method":"send","params":{"to":"+15551234567","reaction_type":"like","reaction_to_guid":"msg-guid-1","send_mode":"applescript"}}"#
  await server.handleLineForTesting(line)
...
@Test
func rpcSendParsesReaction() async throws {
  let store = try RPCTestDatabase.makeStore()
  let output = TestRPCOutput()
  var captured: MessageSendOptions?
  let server = RPCServer(
  ...
  let line =
    #"{"jsonrpc":"2.0","id":"7","method":"send","params":{"to":"+15551234567","reaction_type":"laugh","reaction_to_guid":"msg-guid-1","send_mode":"imcore"}}"#
  await server.handleLineForTesting(line)

  #expect(captured?.reactionType == .laugh)
